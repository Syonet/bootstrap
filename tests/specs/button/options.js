(function( $ ) {
	...
	test( "multi", 2, function() {
		...
		strictEqual( this.buttonset.find(".syo-button.syo-active").length, 3, "quando opção = true, deve poder ativar mais de um botão ao mesmo tempo" );

		this.buttonset.syoButton( "option", "multi", false );
		this.buttonset.find(".syo-button").trigger("click");
		strictEqual( this.buttonset.find(".syo-button.syo-active").length, 1, "quando opção = false, deve poder ativar apenas um botão ao mesmo tempo" );
	});
	...
	test( "toggling", 3, function() {
		...
		ok( !this.button.is(".syo-active"), "quando opção setada para false, deve desativar o botão" );

		var buttons = this.buttonset.find(".syo-button").trigger("click");
		this.buttonset.syoButton( "option", "toggling", false );

		ok(
			buttons.eq( 0 ).is(".syo-active") && ( buttons.filter(".syo-active:gt(0)" ).length === 0 ),
