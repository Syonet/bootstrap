(function( $ ) {
	...
	test( "disable", 3, function() {
		...
		strictEqual( this.pagination.find( "li.syo-disabled" ).length, 2, "deve desabilitar a lista de indices " +
				"passada por parâmetro" );
		this.pagination.syoPagination( "refresh" );
		
		this.pagination.syoPagination( "disable" );
		strictEqual( this.pagination.find( "li.syo-disabled" ).length, 3, "deve desabilitar todos os itens, se não " +
				"passar parâmetro" );
	});
	...
	test( "enable", 3, function() {
		...
		ok( this.pagination.find( "li" ).eq( 1 ).is( ":not(.syo-disabled)" ), "deve habilitar o indice passado por " +
				"parâmetro" );
		this.pagination.syoPagination( "refresh" ).syoPagination( "disable" );
		
		this.pagination.syoPagination( "enable", [ 0, 1 ] );
		strictEqual( this.pagination.find( "li:not(.syo-disabled)" ).length, 2, "deve habilitar a lista de indices " +
				"passada por parâmetro" );
		this.pagination.syoPagination( "refresh" ).syoPagination( "disable" );
		
		this.pagination.syoPagination( "enable" );
		strictEqual( this.pagination.find( "li:not(.syo-disabled)" ).length, 3, "deve habilitar todos os itens, se " +
				"não passar parâmetro" );
	});
