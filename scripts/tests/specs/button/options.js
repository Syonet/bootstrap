(function( $ ) {
	...
	test( "multi", 2, function() {
		...
		strictEqual( this.buttonset.find(".syo-button.syo-active").length, 3, "quando opção = true, deve poder " +
				"ativar mais de um botão ao mesmo tempo" );
		
		this.buttonset.syoButton( "option", "multi", false );
		this.buttonset.find( ".syo-button" ).trigger( "click" );
		strictEqual( this.buttonset.find(".syo-button.syo-active").length, 1, "quando opção = false, deve poder " +
				"ativar apenas um botão ao mesmo tempo" );
	});
